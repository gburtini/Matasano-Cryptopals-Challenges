...
async function challenge() {
  ...
  function server(file, signature) {
    async function insecureCompare(a, b, delayMean = 5, delayVariance = 1) {
  ...
  async function timeGuess(guess) {
    // NOTE: replacing this guy with one that samples 10 times is the only change
    // between 31 and 32. Wrap the code better so we don't have all this repetition.
    const readings = [];
    for (let i = 0; i < 10; i++) {
      const before = Date.now();
      await server(fileTested, guess);
      const after = Date.now();
      readings.push(after - before);
    }
...
module.exports = {
  run: challenge,
  describe: 'Break HMAC-SHA1 with a slightly less artificial timing leak',
};
