...
function xor(a, b) {
  ...
  if (a.length !== b.length) throw new Error(`a (${a.length}) and b (${b.length}) do not have same length.`);
  const retVal = [];
...
function xorRepeatedKey(a, b) {
  ...
  const longerKey = b.toString('ascii')
    .repeat(Math.ceil(a.length / b.length));
