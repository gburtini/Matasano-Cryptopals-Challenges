...
// TODO: split this into a subdirectory.
...
function breakRepeatingKeyXor(ciphertext, minKeySize = 2, maxKeySize = 40) {
  ...
  if (minKeySize <= 0) {
    throw new Error('Senseless minimum key size. Set to an integer greater than 0.');
  }
  if (maxKeySize < minKeySize) {
    throw new Error('Senseless key sizes: maximum key size is less than minimum key size.');
  }
  ...
  for (let keySize = minKeySize; keySize < maxKeySize; keySize++) {
    ...
    for (let offset = 0; offset < replicas; offset++) {
      const distance =
        scores.hamming(
          new Buffer(ciphertext.slice(offset * keySize, (offset + 1) * keySize), 'ascii'),
          new Buffer(ciphertext.slice((offset + 1) * keySize, (offset + 2) * keySize), 'ascii')
        ) / keySize;
  ...
  for (let i = 0; i < byteArrays.length; i++) {
    bests.push(
      findMostLikelyPlaintext(new Buffer(byteArrays[i].join(''), 'ascii')).characterCandidate
    );
  }

  // Put the predicted key back together and decrypt.
  const key = bests.join('').toString('ascii');
  return xor.key(ciphertext, key).toString('ascii');
}

function breakKnownPrefixEcb(unknownBlock, cipher, blockSize = 16, padBytes = 0) {
