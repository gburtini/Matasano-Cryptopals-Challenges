...
function breakKnownPrefixEcb(unknownBlock, cipher, blockSize = 16, padBytes = 0) {
  class BadPaddingError extends Error {
    get type() { return 'BadPaddingError'; }
  }
  class FailFirstByteError extends Error {
    get type() { return 'FailFirstByteError'; }
  }
  ...
  for (let j = 0; j < blockSize; j++) {
    ...
    for (let i = 0; i < 256; i++) {
      ...
      dictionary[baseString + extraString] = chunk(
        Array.from(
          cipher(padString + baseString + extraString)
        ),
        blockSize
    ...
    const blockBytes = chunk(
      Array.from(
        cipher(padString + allAs.substring(j + 1), unknownBlock)
      ),
      blockSize
    );
